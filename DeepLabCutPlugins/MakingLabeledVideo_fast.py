...
colors=(C[:,:3]*254).astype(np.uint8)
        ...
        for index in tqdm(range(nframes)):
            ...
            for bpindex, bp in enumerate(bodyparts2plot):
                if Dataframe[scorer][bp]['likelihood'].values[index] > pcutoff:
                    ...
                    image[rr, cc, :] = (image[rr, cc, :]*(1-alphavalue)).astype(np.uint8)
                    image[rr, cc, :] += (colors[bpindex]*alphavalue).astype(np.uint8)
